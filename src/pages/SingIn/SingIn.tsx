...
const SingIn = () => {
    return (
        <div className="flex h-screen w-full">
            ...
            <div className="w-1/2 bg-(--color-primary-800) flex flex-col justify-center items-center p-16 text-(--color-primary-50)">
                <div className="w-full max-w-sm mb-12">
                    <h1 className="text-4xl font-bold flex items-center text-(--color-neutral-100)">
                        Money<span className="text-(--color-primary-600)">Track</span>
                    </h1>
                </div>

                <h2 className="text-4xl text-center text-(--color-neutral-100) font-light mb-6 self-start">
                    Seja bem vindo!
                </h2>

                <p className="text-lg text-(--color-neutral-100) self-start">
                    Contrary to popular belief, Lorem Ipsum is not simply random text.
                </p>
            </div>


            <div className="w-1/2 bg-gray-100 flex flex-col justify-center items-center p-16">

                <h2 className="text-3xl font-semibold text-(--color-primary-500) mb-10">
                    Acesse sua conta
                </h2>

                <form action="" className="w-full max-w-xs">

                    <div className="relative text-(--color-primary-500) focus-within:text-text-(--color-primary-500) mb-6">
                        ...
                        <input
                            type="email"
                            id="email-input"
                            className="block w-full h-11 pr-5 pl-12 py-2.5 text-base font-normal shadow-sm text-text-(--color-primary-800) bg-white border border-gray-300 rounded-lg placeholder-text-(--color-primary-00) focus:outline-none focus:border-(--color-primary-500)"
                            placeholder="Nome"
                        />
                    </div>
                    ...
                    <div className="relative text-gray-500 focus-within:text-gray-900 mb-2">
                        <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none ">
                            ...
                            <svg className="stroke-current ml-1" fill="currentColor" xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 448 512">
                                <path d="M144 144v48H304V144c0-44.2-35.8-80-80-80s-80 35.8-80 80zM80 192V144C80 64.5 144.5 0 224 0s144 64.5 144 144v48h16c35.3 0 64 28.7 64 64V448c0 35.3-28.7 64-64 64H64c-35.3 0-64-28.7-64-64V256c0-35.3 28.7-64 64-64H80z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
                            </svg>
                        </div>
                        <input
                            type="password"
                            id="password-input"
                            className="block w-full h-11 pr-5 pl-12 py-2.5 text-base font-normal shadow-sm text-(--color-primary-500) bg-white border border-gray-300 rounded-lg placeholder-(--color-primary-200) focus:outline-none focus:border-(--color-primary-500) "
                            placeholder="Senha"
                        />
                    </div>

                    <div className="flex justify-start w-full mb-6">
                        <Link to="/recuperar-senha" className="text-sm text-(--color-primary-00)">
                            Esqueceu sua senha? <span className="text-(--color-primary-500) font-medium">Recuperar</span>
                        </Link>
                    </div>

                    <div className="flex mb-8">
                        <button type="submit" className="w-full h-12 bg-(--color-primary-500) hover:bg-(--color-primary-800) transition-all duration-300 rounded-lg shadow-md text-white text-base font-semibold leading-6 cursor-pointer">
                            Entrar
                        </button>
                    </div>

                    <div className="flex w-full items-center space-x-4 mt-4 mb-4">
                        <div className="flex-1 border-b border-gray-300"></div>
                        <span className="flex items-center justify-center text-gray-500 text-sm font-normal leading-7 px-3">OU</span>
                        <div className="flex-1 border-b border-gray-300"></div>
                    </div>

                    <p className="text-gray-900 text-center text-base font-medium leading-6 mb-4">Entrar com redes sociais</p>
                    <div className="flex items-center justify-center gap-x-3 mb-8">

                        <a href="javascript:;" className="relative w-9 h-9 rounded-full transition-all duration-500 flex justify-center items-center bg-gray-700 hover:bg-indigo-600">
                    ...
                    <p className="text-center text-sm text-gray-600">
                        Não tem conta?
                        <Link to="/cadastro" className="text-indigo-600 font-medium ml-1 hover:text-indigo-800">Cadastre-se</Link>
                    </p>
